...
// StarRocksReaderJob StarRocks读取作业
// StarRocks使用MySQL协议兼容，但有自己的特性和优化
type StarRocksReaderJob struct {
...
func NewStarRocksReaderJob() *StarRocksReaderJob {
	return &StarRocksReaderJob{
		factory: factory.GetGlobalFactory(),
	}
}

func (job *StarRocksReaderJob) Init(config config.Configuration) error {
	job.config = config
	...
	if job.querySql != "" {
		...
		logger.Component().WithComponent("StarRocksReader").Info("StarRocks Reader initialized with querySql mode")
	} else {
		...
		logger.Component().WithComponent("StarRocksReader").Info("StarRocks Reader initialized with table/column mode",
...
func (job *StarRocksReaderJob) Split(adviceNumber int) ([]config.Configuration, error) {
	...
	if job.querySql != "" || job.splitPk == "" {
		...
		if job.querySql != "" {
			logger.Component().WithComponent("StarRocksReader").Info("Using querySql mode, single task")
		} else {
			logger.Component().WithComponent("StarRocksReader").Info("No splitPk specified, using single task")
		}
	...
	if err != nil {
		logger.Component().WithComponent("StarRocksReader").Warn("Failed to calculate split ranges, fallback to single task",
			zap.Error(err))
		taskConfig := job.config.Clone()
		taskConfig.Set("taskId", 0)
		taskConfigs = append(taskConfigs, taskConfig)
	...
	logger.Component().WithComponent("StarRocksReader").Info("Split into tasks based on splitPk",
		zap.Int("taskCount", len(taskConfigs)),
		zap.String("splitPk", job.splitPk))
	return taskConfigs, nil
}

func (job *StarRocksReaderJob) calculateSplitRanges(adviceNumber int) ([]map[string]interface{}, error) {
	// 连接数据库
	db, err := job.connect()
	if err != nil {
		return nil, err
...
// detectColumnType 检测列的数据类型（StarRocks版本）
// StarRocks兼容MySQL的information_schema
func (job *StarRocksReaderJob) detectColumnType(db *gorm.DB, table, column string) (string, error) {
	query := `
		SELECT DATA_TYPE
...
// isNumericType 判断是否为数值类型（StarRocks版本）
// StarRocks支持更多数值类型
func (job *StarRocksReaderJob) isNumericType(dataType string) bool {
	numericTypes := []string{
		"int", "integer", "bigint", "smallint", "tinyint", "mediumint",
		"decimal", "numeric", "float", "double", "bit",
		// StarRocks特有数值类型
		"largeint", "decimalv2", "decimal32", "decimal64", "decimal128",
	}
...
// isTextType 判断是否为文本类型（StarRocks版本）
func (job *StarRocksReaderJob) isTextType(dataType string) bool {
	textTypes := []string{
...
// calculateNumericSplitRanges 计算数值类型的分片范围（StarRocks版本）
func (job *StarRocksReaderJob) calculateNumericSplitRanges(db *gorm.DB, adviceNumber int) ([]map[string]interface{}, error) {
	table := job.tables[0]
...
// calculateTextSplitRanges 计算文本类型的分片范围（StarRocks版本）
func (job *StarRocksReaderJob) calculateTextSplitRanges(db *gorm.DB, adviceNumber int, columnType string) ([]map[string]interface{}, error) {
	// 策略1: 尝试字典序范围切分
	ranges, err := job.calculateTextDictionarySplitRanges(db, adviceNumber)
	if err != nil {
		logger.Component().WithComponent("StarRocksReader").Warn("Dictionary-based splitting failed, trying offset-based splitting",
			zap.Error(err))

		// 策略2: 使用OFFSET/LIMIT切分
		ranges, err = job.calculateOffsetSplitRanges(db, adviceNumber)
		if err != nil {
			logger.Component().WithComponent("StarRocksReader").Warn("Offset-based splitting failed, falling back to hash-based splitting",
				zap.Error(err))
			// 策略3: hash切分（兜底）
...
// calculateTextDictionarySplitRanges 基于字典序的文本切分（StarRocks版本）
func (job *StarRocksReaderJob) calculateTextDictionarySplitRanges(db *gorm.DB, adviceNumber int) ([]map[string]interface{}, error) {
	table := job.tables[0]
...
// calculateOffsetSplitRanges 基于OFFSET/LIMIT的切分（StarRocks版本）
func (job *StarRocksReaderJob) calculateOffsetSplitRanges(db *gorm.DB, adviceNumber int) ([]map[string]interface{}, error) {
	table := job.tables[0]
...
// calculateHashSplitRanges 基于hash的切分（StarRocks版本）
func (job *StarRocksReaderJob) calculateHashSplitRanges(adviceNumber int) []map[string]interface{} {
	ranges := make([]map[string]interface{}, 0)
...
func (job *StarRocksReaderJob) connect() (*gorm.DB, error) {
	// 构建连接字符串
	jdbcUrl := job.jdbcUrls[0]

	// 转换JDBC URL为MySQL兼容的连接字符串
	dsn, err := job.convertJdbcUrl(jdbcUrl)
	if err != nil {
		return nil, err
	}

	// 连接数据库 - 使用MySQL驱动（StarRocks兼容MySQL协议）
...
func (job *StarRocksReaderJob) convertJdbcUrl(jdbcUrl string) (string, error) {
	// 解析JDBC URL: jdbc:mysql://host:port/database?参数
	// StarRocks使用MySQL兼容协议
...
func (job *StarRocksReaderJob) Post() error {
	return nil
}

func (job *StarRocksReaderJob) Destroy() error {
	return nil
}

// StarRocksReaderTask StarRocks读取任务
type StarRocksReaderTask struct {
	config    config.Configuration
	readerJob *StarRocksReaderJob
	db        *gorm.DB
	factory   *factory.DataXFactory
}

func NewStarRocksReaderTask() *StarRocksReaderTask {
	return &StarRocksReaderTask{
		factory: factory.GetGlobalFactory(),
	}
}

func (task *StarRocksReaderTask) Init(config config.Configuration) error {
	task.config = config

	// 创建ReaderJob来重用连接逻辑
	task.readerJob = NewStarRocksReaderJob()
...
func (task *StarRocksReaderTask) StartRead(recordSender plugin.RecordSender) error {
	defer func() {
	...
	logger.Component().WithComponent("StarRocksReader").Info("Executing query",
	...
	for rows.Next() {
		...
		if recordCount%1000 == 0 {
			logger.Component().WithComponent("StarRocksReader").Info("Reading progress",
				zap.Int("recordCount", recordCount))
		}
	}

	logger.Component().WithComponent("StarRocksReader").Info("Reading completed",
		zap.Int("totalRecords", recordCount))
	return nil
}

func (task *StarRocksReaderTask) buildQuery() (string, error) {
...
func (task *StarRocksReaderTask) buildSplitCondition(rangeMap map[string]interface{}) (string, string, string) {
...
func (task *StarRocksReaderTask) buildNumericCondition(rangeMap map[string]interface{}) (string, string, string) {
...
func (task *StarRocksReaderTask) buildTextDictionaryCondition(rangeMap map[string]interface{}) (string, string, string) {
	start := rangeMap["start"]
	end := rangeMap["end"]
...
func (task *StarRocksReaderTask) buildOffsetCondition(rangeMap map[string]interface{}) (string, string, string) {
...
func (task *StarRocksReaderTask) buildHashCondition(rangeMap map[string]interface{}) (string, string, string) {
	taskId := rangeMap["taskId"]
	total := rangeMap["total"]

	// StarRocks使用CRC32函数进行hash（兼容MySQL）
...
func (task *StarRocksReaderTask) convertToColumn(value interface{}) element.Column {
...
func (task *StarRocksReaderTask) Post() error {
	return nil
}

func (task *StarRocksReaderTask) Destroy() error {
	if task.db != nil {
		if sqlDB, err := task.db.DB(); err == nil {
			sqlDB.Close()
		}
